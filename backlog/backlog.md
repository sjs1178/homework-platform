...
#### 배포 전 수정
- [x] AI 면책 안내 (자녀 통계 직업 가이드)
- [x] 캘린더 chevron-left 아이콘 누락 수정
- [x] 최소 가입 연령 만 7세 → 만 5세 변경
- [x] 약관 전문 뒤로가기 수정 (PageHeader 히스토리 폴백)
- [x] 만 14세 미만 동의 단계 생략 (개인정보보호법 제22조의2)
- [x] 미사용 승인 코드 표시 제거
- [x] 숙제 검사 카드 높이 80% 축소
- [x] 숙제 요청 버튼 대시보드→캘린더/숙제입력 이동
- [x] kiddoloop 투톤 워드마크 (kiddo=#13241B, loop=#16A34A)
- [x] safe-area-inset-bottom 패턴 변경

#### 디자인 통일
- [x] 자녀 전 페이지 Tailwind/blue → inline styles/브랜드 그린 변환
- [x] 로그아웃 버튼 부모/자녀 통일 (Icon log-out + danger 스타일)

#### 이메일 발송
- [x] 가입 이메일 자동 발송 (Nodemailer + Zoho SMTP)
  - 자녀→부모 승인 요청 메일
  - 부모→자녀 승인 완료 메일
- [x] `src/lib/send-email.ts` 이메일 유틸리티 신설
