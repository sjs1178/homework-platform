...
Google 로그인
  ...
  ├── Step 2: 생년월일 입력 → 만 5세 미만 가입 불가 / 성인·미성년자 자동 판별
  │
  ├── [성인 경로 — 만 19세 이상]
  ...
  │
  ├── [미성년자 경로 — 만 14세~18세]
  │     ├── 이용약관 + 개인정보처리방침 동의 (본인 동의)
  │     ├── 부모님 구글 계정 이메일 입력
  │     ├── pending_approvals 레코드 생성 → 부모 이메일 자동 발송
  │     └── 대기 화면 표시 (만료일)
  │
  └── [미성년자 경로 — 만 14세 미만]
        ├── 동의 단계 생략 (법정대리인이 대리 동의)
        ├── 부모님 구글 계정 이메일 입력
        ├── pending_approvals 레코드 생성 → 부모 이메일 자동 발송
        └── 대기 화면 표시 (만료일)
              ↓
        [부모님 이메일 수신 → 승인 링크 클릭 또는 설정 → "가입 승인 대기" 섹션]
              ├── 자녀 이름·생년월일 확인
              ├── 법정대리인 약관 동의 선택
              └── 승인 → 자녀 프로필 생성 + 자동 페어링 → 자녀 이메일로 승인 완료 알림
```

**법령 준수 근거:**
- 만 5세 미만: 가입 불가 (서비스 최소 이용 연령)
- 만 5세~13세 (만 14세 미만): 개인정보보호법 제22조의2 — 법정대리인 동의만 필요 → 자녀 본인 동의 생략, 부모가 대리 동의
...
**이메일 알림:**
- 자녀 가입 요청 시 → 부모 이메일로 승인 요청 메일 자동 발송 (승인 페이지 링크 포함)
- 부모 승인 완료 시 → 자녀 이메일로 가입 승인 확인 메일 발송 (로그인 링크 포함)
- SMTP: Zoho Mail (contact@kiddoloop.com), Nodemailer 사용
...
대시보드 (헤더: kiddoloop [자녀] 뱃지)
  ...
  │     └── 부모님께 숙제 등록 요청 버튼
  │
  ├── 숙제 등록 요청 (`/child/homework-request`)
  │     ├── 숙제 사진 촬영/업로드
  │     ├── 부모님께 메시지 (선택)
  │     └── 전송 → 부모 숙제 입력 화면에 요청 알림 배너
  │
  ├── 학습 통계 (`/child/stats`)
  │     ├── 과목별 완료 현황 (브랜드 그린 디자인)
  │     ├── 직업군 가이드 (AI 생성, 면책 안내 포함)
  │     └── AI 기반 학습 로드맵
  │
  ├── 내 리워드
  │     ├── 잔액 히어로 카드 + 프로그레스 바
...
| 미성년자 부모 승인 흐름 | ✅ | 부모 이메일 입력→pending_approval→이메일 발송→부모 설정 화면 승인 |
| 만 14세 미만 동의 생략 | ✅ | 개인정보보호법 제22조의2: 법정대리인만 동의, 자녀 본인 동의 단계 skip |
| 부모 동시 가입 + 자동 페어링 | ✅ | 승인 시 자녀 프로필 생성 + 페어 자동 연결 |
| 가입 이메일 자동 발송 | ✅ | 자녀→부모 승인 요청 메일 + 부모→자녀 승인 완료 메일 (Nodemailer + Zoho SMTP) |
...
| 승인 대기 화면 | ✅ | 미성년자 대기 화면에 만료일 표시 + 부모 이메일 발송 안내 |
...
| `/child/calendar` | 월간 숙제 캘린더 (다:다 통합) + 숙제 등록 요청 링크 |
| `/child/homework-request` | 부모에게 숙제 등록 요청 (사진+메시지) |
| `/child/stats` | 학습 통계 (과목별 완료 현황 + 직업군 가이드) |
| `/child/results` | 채점 결과 확인 (점수·해설) |
| `/child/rewards` | 리워드 히어로·내역·교환 카탈로그 |
| `/child/profile` | 프로필 편집 (이름·아바타·로그아웃) |
...
| `/api/onboarding/request-parent` | POST | 미성년자 부모 승인 요청 생성 + 부모 이메일 발송 |
| `/api/onboarding/approve-child` | POST | 부모가 자녀 승인 → 프로필 생성 + 자동 페어링 + 자녀 이메일 발송 |
| `/api/homework-request` | POST | 자녀 숙제 등록 요청 (create/list/update) |
...
| 2026-06-20 | 최소 가입 연령 만 7세 → 만 5세로 변경 | 유치원생 사용자 커버, 부모 동의 흐름으로 보호 충분 |
| 2026-06-20 | 만 14세 미만 동의 단계 생략 | 개인정보보호법 제22조의2: 법정대리인만 동의하면 되므로 자녀 본인 동의 불필요 |
| 2026-06-20 | 가입 이메일 발송 Nodemailer + Zoho SMTP | 인앱 승인코드만으로는 부모 인지 어려움, 이메일로 직접 알림 |
| 2026-06-20 | 이메일 발송 실패 시 API 응답에 영향 없게 처리 | `.catch(console.error)` — SMTP 장애가 가입 흐름을 막지 않도록 |
| 2026-06-20 | 자녀 페이지 디자인 부모 페이지와 통일 | Tailwind/blue 잔재 제거, 브랜드 그린 + inline styles 일관 적용 |
| 2026-06-20 | kiddoloop 워드마크 투톤 적용 (kiddo=#13241B, loop=#16A34A) | 로고 가이드 준수, 브랜드 아이덴티티 강화 |

---

## 14. 현재 구현 완성도 (v0.2, 2026-06-20 기준)
...
- [x] 만 5세 미만 가입 차단 (최소 이용 연령)
- [x] 만 14세 미만 동의 단계 생략 (법정대리인 대리 동의)
- [x] 가입 이메일 자동 발송 (부모 승인 요청 + 자녀 승인 완료)
...
- [x] 월간 캘린더 (날짜별 숙제 현황)
- [x] 숙제 완료 처리
- [x] 부모에게 숙제 등록 요청 (사진+메시지)
- [x] 학습 통계 (과목별 완료 현황 + 직업군 가이드)
- [x] 채점 결과 확인
- [x] 이모지 아바타 프로필
- [x] 리워드 잔액·내역 확인
- [x] 로그아웃
...
- [x] Nodemailer SMTP 이메일 발송 (가입 프로세스)
